...
def ForgeryDetection():
    global shufflenet
    labels = ['Original Image Detected', 'Forgery Image Detected']
    ...
    img = img/255
    preds = squeezenet.predict(img)
    predict = np.argmax(preds)
    print(predict)
    img = cv2.imread(filename)
    img = cv2.resize(img, (700,400))
    cv2.putText(img, labels[predict], (10, 25),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 0, 255), 2)
...
predictButton = Button(main, text="Forgery Detection from Test Image", command=ForgeryDetection)
